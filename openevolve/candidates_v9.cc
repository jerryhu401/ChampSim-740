...
ipcp::ip_class_t ipcp::classify_ip(int64_t old_stride, int64_t new_stride, int confidence, ip_class_t old_class)
{
  ...
  if (global_stream_dir != 0) {
    ...
    // Apply the magnitude gate ONLY for never-confirmed IPs (confidence==0).
    // Those are the source of the mcf / omnetpp pollution. Confirmed IPs
    // (any confidence) are trusted regardless of stride magnitude.
    bool stride_ok = (confidence > 0) || (std::abs(new_stride) <= GS_STRIDE_LIMIT);
    if (dir_match && stride_ok) {
