...
ipcp::ip_class_t ipcp::classify_ip(int64_t old_stride, int64_t new_stride, int confidence, ip_class_t old_class)
{
  ...
  // CPLX disabled (v13). Signature-table predictions burn bandwidth on
  // memory-bound workloads (mcf/omnetpp) without a clear win, so unstable
  // strides fall straight through to the conservative NL path.
  return NL;
}
// EVOLVE-BLOCK-END
