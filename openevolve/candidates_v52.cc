                                        ...
                                        uint32_t metadata_in)
{
  ...
  case GS:
    // v52: keep the ±1 stream chain AND add an extra stride-aware chain.
    if (global_stream_dir != 0)
      issue_prefetch(addr, block, global_stream_dir, GS_DEGREE, metadata_in);
    if (new_stride != 0 && std::abs(new_stride) > 1)
      issue_prefetch(addr, block, new_stride, CS_DEGREE / 2, metadata_in);
    break;
  case NL:
