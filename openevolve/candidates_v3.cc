...
ipcp::ip_class_t ipcp::classify_ip(int64_t old_stride, int64_t new_stride, int confidence, ip_class_t old_class)
{
  ...
  // GS shortcut: drop the small-stride filter (it killed xalancbmk in v2).
  // Instead, require the IP to have one prior stable observation
  // (confidence >= 1) OR be already classified as GS. This protects mcf /
  // omnetpp (where many *fresh* IPs naturally drift in the global direction
  // but aren't actually streaming) without losing fresh streaming IPs on
  // gcc / xalancbmk that quickly accumulate confidence.
  if (global_stream_dir != 0 && (confidence >= 1 || old_class == GS)) {
    bool dir_match = (global_stream_dir > 0 && new_stride > 0) || (global_stream_dir < 0 && new_stride < 0);
    if (dir_match && (confidence < CS_CONFIDENCE_THRESHOLD || old_class == GS)) {
      return GS;
    }
  }
