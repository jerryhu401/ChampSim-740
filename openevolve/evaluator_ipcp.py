"""
OpenEvolve evaluator for the IPCP prefetcher in ChampSim.

Flow per evaluation:
  1. Copy the evolved program file to prefetcher/ipcp/ipcp.cc
  2. Run config.sh + make to rebuild bin/champsim_ipcp
...
SRC_CC = REPO / "prefetcher" / "ipcp" / "ipcp.cc"
BINARY = REPO / "bin" / "champsim_ipcp"
...
def evaluate(program_path: str) -> dict:
    ...
    if IS_WSL:
        ...
        # 2a. Regenerate _configuration.mk for champsim_ipcp
        cfg = subprocess.run(
            [cmd, "/c",
             path_prefix + cd_prefix +
             "C:\\msys64\\mingw64\\bin\\python3.exe config.sh champsim_config_ipcp.json"],
        ...
        build = subprocess.run(
            [cmd, "/c",
             path_prefix + cd_prefix + "mingw32-make -j4 bin/champsim_ipcp"],
        ...
        cfg = subprocess.run(
            [r"C:\msys64\mingw64\bin\python3.exe", "config.sh", "champsim_config_ipcp.json"],
        ...
        stale = REPO / "bin" / "champsim_ipcp.exe"
        if stale.exists():
            stale.unlink()
