...
const std::array<int, 26> bop::OFFSET_LIST = {
    1, 2, 3, 4, 5, 6, 8, 9, 10, 12, 15, 16, 18, 20, 24, 25, 27, 30, 32, 36, 40, 45, 48, 50, 54, 60};

// === BEGIN EVOLVABLE FUNCTION ===
int bop::score_update(int old_score)
{
  return old_score + 1;
}
                                       ...
                                       uint32_t metadata_in)
{
  ...
  // --- Learning phase: test one candidate offset per access ---
  ...
  // --- Insert current block into RR table ---
  rr_table[rr_hash(block_val)] = block_val;

  // --- Issue prefetches using current best offset ---
  if (best_offset != 0) {
...
{
  // Insert prefetched blocks into RR table to improve offset learning
  if (prefetch) {
    champsim::block_number block{addr};
    uint64_t block_val = block.to<uint64_t>();
    rr_table[rr_hash(block_val)] = block_val;
